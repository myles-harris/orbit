...
// The short date in the device's own locale ("Sep 17", or "17 Sept" in en-GB) — built the
// way the code under test builds it, so the suite does not assume a locale.
const shortDate = (day: number) => new Date(2026, 8, day).toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
...
describe('formatSavedAt', () => {
  it('is just the time for a copy from today', () => {
    ...
    expect(out).not.toContain(shortDate(19));
  });
  ...
  it('reads a copy from later than now as another day rather than as today', () => {
    ...
    expect(formatSavedAt(at(21, 9, 41), at(19, 14, 5))).toContain(shortDate(21));
  });
});
