 ...
 */
export function toViewerWindow(windowStartHour, windowEndHour, groupTz, viewerTz, anchor = new Date()) {
 ...
 */
export function formatViewerWindow(windowStartHour, windowEndHour, groupTz, viewerTz, anchor) {
    if (groupTz === viewerTz) {
        return `${formatMinutes(windowStartHour * 60)} – ${formatMinutes(windowEndHour * 60)}`;
    }
    const { startMinutes, endMinutes, dayShift } = toViewerWindow(windowStartHour, windowEndHour, groupTz, viewerTz, anchor);
    const start = formatMinutes(startMinutes);
    const end = formatMinutes(endMinutes);
    if (dayShift === 1)
        return `${start} – ${end} (next day)`;
    if (dayShift === -1)
        return `${start} – ${end} (prev day)`;
    return `${start} – ${end}`;
}
